...
- **프론트엔드 클라이언트 타입** — `nestjs-safe-response/client`에서 런타임 의존 없는 TypeScript 타입 + 타입 가드 (`isSuccess`, `isError`, `isPaginated`, `isProblemDetailsResponse`, `hasResponseTime`, `hasSort`, `hasFilters`, `isDeprecated`, `hasRateLimit`) 제공
