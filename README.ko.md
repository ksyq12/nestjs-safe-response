...
- **프론트엔드 클라이언트 타입** — `nestjs-safe-response/client`에서 런타임 의존 없는 TypeScript 타입 + 타입 가드 제공
...
| 카테고리 | 수량 | 검증 범위 |
|----------|------|-----------|
| 단위 테스트 | 278 | Interceptor, Exception Filter, Module DI, Decorators, Client Type Guards, i18n Adapter, Global Errors |
| E2E 테스트 (Express) | 45 | v0.9.0 기능 포함 전체 HTTP 요청/응답 사이클 |
| E2E 테스트 (Fastify) | 14 | v0.9.0 기능 포함 플랫폼 동일 동작 검증 |
| E2E 테스트 (Swagger) | 41 | Problem Details, Global Errors 포함 OpenAPI 스키마 출력 검증 |
| 타입 테스트 | 72 | `tsd`로 Public API 타입 시그니처 검증 (클라이언트 타입 + v0.9.0 옵션 포함) |
