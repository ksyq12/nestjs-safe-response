...
describe('SafeResponse E2E (Fastify)', () => {
  ...
  describe('커스텀 에러 코드', () => {
    it('errorCodeMapper로 커스텀 코드 반환', async () => {
      app = await createFastifyApp(TestAppCustomErrorCodeModule);
  ...
  describe('옵션 조합', () => {
    it('timestamp: false, path: false → 두 필드 모두 없음', async () => {
      app = await createFastifyApp(TestAppNoMetaModule);
    ...
    it('dateFormatter 커스텀 → 성공/에러 모두 해당 포맷', async () => {
      app = await createFastifyApp(TestAppCustomDateModule);
  ...
  describe('transformResponse', () => {
    beforeEach(async () => {
      app = await createFastifyApp(TestAppTransformModule);
    });
  ...
  describe('성공 코드 매핑', () => {
    it('successCodeMapper → 응답에 code 필드 포함', async () => {
      app = await createFastifyApp(TestAppSuccessCodeModule);
    ...
    it('@SuccessCode() 우선순위: 데코레이터 > 전역 매퍼', async () => {
      app = await createFastifyApp(TestAppSuccessCodePriorityModule);
  ...
  describe('엣지 케이스: 비정형 데이터', () => {
    ...
    it('GET /test/edge-buffer → Buffer가 JSON 직렬화됨', async () => {
      const res = await request(app.getHttpServer())
  ...
  describe('@Exclude() 공존', () => {
    it('정상 순서: @Exclude() 필드가 응답에서 제거됨', async () => {
      app = await createFastifyApp(TestAppExcludeModule);
  ...
  describe('요청 ID', () => {
    it('requestId: true → 응답에 requestId 포함 + X-Request-Id 헤더', async () => {
      app = await createFastifyApp(TestAppRequestIdModule);
    ...
    it('커스텀 헤더명 X-Correlation-Id', async () => {
      app = await createFastifyApp(TestAppCustomRequestIdModule);
  ...
  describe('RFC 9457 Problem Details', () => {
    ...
    it('baseUrl 설정 → type에 baseUrl 접두사', async () => {
      app = await createFastifyApp(TestAppProblemDetailsBaseUrlModule);
      const res = await request(app.getHttpServer())
        .get('/test/not-found')
        .expect(404);
    ...
    it('requestId + responseTime + problemDetails → 모두 포함', async () => {
      app = await createFastifyApp(TestAppProblemDetailsFullModule);
      const res = await request(app.getHttpServer())
        .get('/test/not-found')
        .expect(404);
